            ...
            <div class="product-view">
              <div class="row">
                ...
                <h5 class="card-title bg-black text-white">Reviews On {{$v->fullName}}</h5>
                  @php
                  if($cover_img->image)
                  $url = asset('uploads/vehicle_images/'.$cover_img->image);
                  else
                  $url = asset('front/img/product-img.png');
                  @endphp
                  <div class="yotpo yotpo-main-widget"  data-product-id="{{$v->stock_id}}" data-price="{{$price_after_dis}}" data-currency="{{$location['geoplugin_currencyCode']}}" data-name="{{ str_replace('-', ' ', $v->name) }}" data-url="{{route('singleVehicle',['brand'=>$brand->slug_name,'subBrand'=>$sub_brand_id->slug_name,'stock_id'=>$v->stock_id])}}" data-image-url="{{$url}}"></div>
                </div>
