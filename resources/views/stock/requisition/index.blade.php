@extends('layout.app')
@section('pageTitle','Requisition List')
@section('pageSubTitle','Requisition List')
@section('content')
    ...
    <section class="section">
        <div class="row" id="table-bordered">
            <div class="col-12">
                <div class="card">
                    <div class="float-end">
                        <a class="btn btn-sm btn-primary float-end ms-3" href="{{route(currentUser().'.requisition.create')}}"><i class="bi bi-plus-square"></i> Add New</a>
                    </div>
                    <!-- table bordered -->
                        ...
                        <table class="table table-bordered mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$data->req_date?\Carbon\Carbon::parse($data->req_date)->format('d-M-y'):""}}</td>
                                    <td>{{$data->slip_no}}</td>
                                    <td>{{$data->line_no}}</td>
                                    <td>{{$data->issue_by}}</td>
                                    ...
                                    <td class="white-space-nowrap">
                                        @if($data->status!=2)
                                        <a href="{{route(currentUser().'.requisition.edit',encryptor('encrypt',$data->id))}}">
                                        ...
                                        @endif
                                        <a href="{{route(currentUser().'.requisition.show',encryptor('encrypt',$data->id))}}">
                                            <i class="bi bi-eye"></i>
                                        </a>
