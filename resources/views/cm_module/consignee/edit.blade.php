...
@section('pageTitle','Update Consignee')
...
<div class="card">
    <div class="col-12">
        ...
        <form class="form" method="post" enctype="multipart/form-data" action="{{route(currentUser().'.consigdetl.update',encryptor('encrypt', $consignee->id))}}">
            @csrf
            @method('patch')
            <input type="hidden" name="uptoken" value="{{encryptor('encrypt',$consignee->id)}}">
            <input type="hidden" name="user_id" value="{{$consignee->user_id}}">
            <div class="row">
